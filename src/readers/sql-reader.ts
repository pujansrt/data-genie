 ...
 */
export class SQLReader implements DataReader {
  ...
  constructor(dbClient: SqlConnection, query: string, params: any[] = []) {
  ...
  private async *readAll(): AsyncIterableIterator<DataRecord> {
    const result = await this.dbClient.query(this.query, this.params);
    const rows = Array.isArray(result) ? result : (result?.rows || []);
    for (const row of rows) yield row;
  }

  private async *readInChunks(): AsyncIterableIterator<DataRecord> {
    let offset = 0;
    let hasMore = true;
